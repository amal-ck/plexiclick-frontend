...
import { clearErrors, deleteProduct, deleteVariant, getAdminProducts, getAdminVariants, getAdminVariantsofProduct, getWarningStock } from '../../actions/productAction';
...
const WarningStock = () => {
    ...
    const { warningStock = [], error } = useSelector((state) => state.warningStock);
    ...
    useEffect(() => {
        ...
        dispatch(getWarningStock(accessToken));
    }, [dispatch, error, isDeleted, deleteError, enqueueSnackbar, accessToken]);
    ...
    warningStock && warningStock.forEach((item) => {
        rows.unshift({
            id: item.id,
